...
namespace Poker.Core.Tests.Evaluation
{
    ...
    public class StraightTests
    {
        ...
        public static IEnumerable<object[]> StraightSets =>
            new List<object[]>
            {
                new object[]
                {
        ...
        [Theory]
        [MemberData(nameof(StraightSets))]
        public void EvaluateBestHand_ReturnsStraights( Card[] sevenCards )
        {
            var hole = new List<Card> { sevenCards[0], sevenCards[1] };
            var board = sevenCards.Skip(2).Take(5).ToList();

            var distinctValues = sevenCards
            ...
            var bestStraightCards = straightValues
                .Select(v => sevenCards
            ...
            baseResult.Rank.Should().Be(HandRank.Straight);
            ...
            for ( int i = 0; i < 5; i++ )
            {
                baseResult.Cards[i].Value.Should().Be(bestStraightCards[i].Value);
            }
        }

        [Theory]
        [MemberData(nameof(StraightSets))]
        public void EvaluateBestHand_Straight_PermutationsDontChangeResult( Card[] sevenCards )
        {
            var hole = new List<Card> { sevenCards[0], sevenCards[1] };
            var board = sevenCards.Skip(2).Take(5).ToList();
